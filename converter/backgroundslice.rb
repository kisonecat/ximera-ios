...
Dir.mktmpdir do |dir|
  `#{input_file} burst output #{dir}/page%03d.png`

  png_filenames1 = Dir.new(dir).entries.select{ |x| x.match( /page[0-9]*\.png$/ ) }
  ...
  png_filenames2 = Dir.new(dir).entries.select{ |x| x.match( /tile[0-9]*-[0-9]*\.png$/ ) }
